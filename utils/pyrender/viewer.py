...
class UI:
    def __init__(self, window):
        ...
        self.renderer = PygletProgrammablePipelineRenderer(window)
        imgui.new_frame()  
        imgui.end_frame()
    ...
    def render(self):
        ...
        changed, self.is_enable_dof = imgui.checkbox(
            label="Enable", state=self.is_enable_dof
        )

        changed, self.focus_distance = imgui.slider_float(
        ...
        changed, self.focus_range = imgui.slider_float(
            label="Focus Range",
            value=self.focus_range,
            min_value=0.0,
            max_value=1.0
        )

        changed, self.bokeh_radius = imgui.slider_float(
            label="Bokeh Radius",
            value=self.bokeh_radius,
            min_value=0.0,
            max_value=16.0
        )

        changed, self.aberration = imgui.slider_float(
            label="Aberration",
            value=self.aberration,
            min_value=0.0,
            max_value=0.5
        )
    ...
    def is_want_capture_mouse(self):
...
class Viewer(pyglet.window.Window):
    ...
    - ``cull_faces``: `bool`, If `True`, backfaces will be culled.
      Defaults to `True`.
    - ``point_size`` : float, The point size in pixels. 
      Defaults to 1px.
    ...
    def on_mouse_drag(self, x, y, dx, dy, buttons, modifiers):
        """Record a mouse drag.
        """
        if self._ui is not None and self._ui.is_want_capture_mouse():
            return
    ...
    def _time_event(dt, self):
        ...
        if self._ui is not None:
            self.render_flags['depth_of_field'] = self._ui.is_enable_dof
            self._renderer.focus_distance = self._ui.focus_distance
            self._renderer.focus_range = self._ui.focus_range
            self._renderer.bokeh_radius = self._ui.bokeh_radius
            self._renderer.aberration = self._ui.aberration
    ...
    def _compute_flags(self):
        ...
        if self.render_flags["depth_of_field"]:
            flags |= RenderFlags.DOF
        if self.render_flags['shadows']:
