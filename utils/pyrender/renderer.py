...
class FrameBuffer(object):
    ...
    def configure(self, width, height):
        self._color_attachment, self._depth_attachment = glGenTextures(2)

        glBindTexture(GL_TEXTURE_2D, self._color_attachment)         
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, None);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        glBindTexture(GL_TEXTURE_2D, 0);

        glBindTexture(GL_TEXTURE_2D, self._depth_attachment)
        ...
        glBindFramebuffer(GL_DRAW_FRAMEBUFFER, self._frame_buffer)
        glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, self._color_attachment, 0)
        glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,  GL_TEXTURE_2D, self._depth_attachment, 0)
        glBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0)

        self._dimension = (width, height)

    def resize(self, width, height):
        if self._dimension is (None, None) or width != self._dimension[0] or height != self._dimension[1]:
            self.delete()
    ...
    def bind_read(self):
        glBindFramebuffer(GL_READ_FRAMEBUFFER, self._frame_buffer)

    def blit(self, dst):
        glBindFramebuffer(GL_READ_FRAMEBUFFER, self._frame_buffer)
        glBindFramebuffer(GL_DRAW_FRAMEBUFFER, dst)
        glBlitFramebuffer(
            0, 0, self._dimension[0], self._dimension[1], 0, 0,self._dimension[0], self._dimension[1],
    ...
    def delete(self):
        ...
        if self._color_attachment is not None:
            glDeleteTextures([self._color_attachment])
        if self._depth_attachment is not None:
            glDeleteTextures([self._depth_attachment])
...
class Renderer(object):
    ...
    def __init__(self, viewport_width, viewport_height, point_size=1.0):
        ...
        self.focus_range = 0.2
        self.bokeh_radius = 8.0
        self.aberration = 0.0

        # Optional framebuffer for offscreen renders
        self._main_fb = FrameBuffer()
        self._blit_fb = FrameBuffer()
    ...
    @focus_range.setter
    def focus_range(self, value):
         self._focus_range = value
    
    @property
    def bokeh_radius(self):
        return self._bokeh_radius

    @bokeh_radius.setter
    def bokeh_radius(self, value):
         self._bokeh_radius = value

    @property
    def aberration(self):
        return self._aberration

    @aberration.setter
    def aberration(self, value):
         self._aberration = value
    ...
    def render(self, scene, flags, seg_node_map=None):
        ...
        if not bool(flags & RenderFlags.DEPTH_ONLY or flags & RenderFlags.SEG or flags & RenderFlags.DOF):
        ...
        # Make forward pass
        self._forward_pass(scene, flags, seg_node_map=seg_node_map)
        ...
        self._depth_of_field_pass(scene, flags)
    ...
    def render_text(self, text, x, y, font_name='OpenSans-Regular', font_pt=40, color=None, scale=1.0, align=TextAlign.BOTTOM_LEFT):
    ...
    def _depth_of_field_pass(self, scene, flags):       
        self._configure_depth_of_field_pass_viewport(flags)

        if flags & RenderFlags.DOF:
            program = self._program_cache.get_program(vertex_shader='depth_of_field.vert', fragment_shader='depth_of_field.frag')
            ...
            glBindTexture(GL_TEXTURE_2D, self._main_fb.color_attachment)
            program.set_uniform('u_TextureColor', self._main_fb.color_attachment)

            glActiveTexture(GL_TEXTURE1)
            glBindTexture(GL_TEXTURE_2D, self._main_fb.depth_attachment)
            program.set_uniform('u_TextureDepth', self._main_fb.depth_attachment)

            program.set_uniform("u_ZNear", scene.main_camera_node.camera.znear)
            program.set_uniform("u_ZFar", scene.main_camera_node.camera.zfar)
            program.set_uniform("u_FocusDistance", self.focus_distance)
            program.set_uniform("u_FocusRange", self.focus_range)
            program.set_uniform("u_BokehRadius", self.bokeh_radius)
            program.set_uniform("u_Aberration", self.aberration)
            ...
            self._blit_fb.blit(0)
        else:
            self._main_fb.blit(self._blit_fb.frame_buffer)
            self._main_fb.blit(0)
    ...
    def _get_primitive_program(self, primitive, flags, program_flags):
        ...
        elif flags & RenderFlags.DOF:
            vertex_shader = 'depth_of_field.vert'
            fragment_shader = 'depth_of_field.frag'
        else:
    ...
    def _configure_forward_pass_viewport(self, flags):  
        self._main_fb.resize(self.viewport_width, self.viewport_height)
        self._main_fb.bind_write()
    ...
    def _configure_depth_of_field_pass_viewport(self, flags):
        self._blit_fb.resize(self.viewport_width, self.viewport_height)
        self._blit_fb.bind_write()
    ...
    def _read_main_framebuffer(self, scene, flags):
        width, height = self._main_fb._dimension[0], self._main_fb._dimension[1]

        self._blit_fb.bind_read()
